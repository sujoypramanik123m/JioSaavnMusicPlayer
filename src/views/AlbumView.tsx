...
import { Disc, Play, ArrowLeft, Music } from 'lucide-react';
...
export const AlbumView: React.FC = () => {
  ...
  const passedAlbum: any = activeViewData?.album || null;
  const [album, setAlbum] = useState<AlbumDetails | null>(passedAlbum);
  const [loading, setLoading] = useState<boolean>(!passedAlbum?.songs || passedAlbum.songs.length === 0);

  useEffect(() => {
    let mounted = true;
    if (passedAlbum) {
      setAlbum(passedAlbum);
      if (!passedAlbum.songs || passedAlbum.songs.length === 0) {
        setLoading(true);
        jioSaavnApi
          .getAlbum({ id: passedAlbum.id, link: passedAlbum.url || passedAlbum.link })
          .then(res => {
            if (mounted && res) {
              setAlbum(res);
            }
          ...
          .catch(err => {
            console.warn('Failed to load full album details', err);
          })
          .finally(() => {
  ...
  }, [passedAlbum?.id, passedAlbum?.url]);
  ...
  return (
    <div className="space-y-8 pb-16 animate-fade-in">
      ...
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row items-center md:items-end gap-6 p-6 rounded-3xl bg-gradient-to-b from-zinc-900 to-zinc-950 border border-zinc-800 shadow-2xl">
        <img
          src={imageUrl}
        ...
        <div className="flex-1 text-center md:text-left min-w-0 space-y-3">
          ...
          <h1 className="text-2xl sm:text-4xl font-extrabold text-white tracking-tight">
            {decodeHTMLEntities(album?.name || 'Album')}
          </h1>
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 text-xs text-zinc-400">
            ...
            {album?.playCount && <span>• {formatNumber(album.playCount)} Plays</span>}
          </div>
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-white flex items-center gap-2">
          <Music className="w-4 h-4 text-emerald-400" /> Album Songs ({songs.length})
        </h2>

        {loading ? (
          <div className="py-12 text-center text-zinc-400 space-y-3">
            <div className="w-8 h-8 border-3 border-emerald-500 border-t-transparent rounded-full animate-spin mx-auto" />
            <p className="text-sm font-medium">Fetching album songs...</p>
          </div>
        ) : songs.length === 0 ? (
          <p className="text-sm text-zinc-500 italic">No tracks loaded in this album.</p>
        ) : (
          <div className="space-y-2">
            {songs.filter(s => s && s.id).map((song, idx) => (
              <SongRow key={song.id || `albumsong-${idx}`} song={song} index={idx} playlistQueue={songs} />
            ))}
          </div>
        )}
      </div>
